...
The `trajectory` module defines the interface and implementations for sensor trajectory modeling in the PERSEO framework. It provides an abstract base class [`Trajectory`][perseo_core.geometry.navigation.trajectory.Trajectory] that specifies the contract for position, velocity, and acceleration queries at arbitrary times, along with a concrete [`CubicSplineTrajectory`][perseo_core.geometry.navigation.cubic_spline_trajectory.CubicSplineTrajectory] implementation using Scipy's [`CubicSpline`][scipy.interpolate.CubicSpline] interpolator. All trajectories support vectorized evaluation.
