...
DT = 5
sensor_velocities = np.diff(sensor_positions, axis=0) / DT
sensor_velocities = np.array(sensor_velocities.tolist() + [sensor_velocities[-1, :].tolist()])
time_axis_relative = np.asarray([DT * k for k in range(sensor_positions.size // 3)])
...
class InverseGeocodingBistaticTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # inputs
        self.init_guess = PreciseDateTime.from_utc_string("17-FEB-2020 16:07:04.908999712209")
        ...
        # expected results
        self.azimuth_res = PreciseDateTime.from_utc_string("17-FEB-2020 16:07:09.730322672472")
        self.range_res = 1.9383737324574213e-05
    ...
    def test_inverse_geocoding_bistatic_case4d(self) -> None:
        ...
        az_times, rng_times = inverse_geocoding_bistatic(
            ...
            frequencies_doppler_centroid=np.repeat(self.doppler_freq, self.N),
            wavelength=self.wavelength,
            init_guess_search_time_step=DT,
        )
...
class InverseGeocodingBistaticInitTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # expected results
        self.result = PreciseDateTime.from_utc_string("17-FEB-2020 16:07:09.908999712209")
    ...
    def test_inverse_geocoding_bistatic_init_core_case0a(self) -> None:
        ...
        az_times = inverse_geocoding_bistatic_init_core(
            trajectory_rx=self.trajectory,
            trajectory_tx=self.trajectory,
            time_axis_rx=time_axis_relative + time_axis_origin,
            time_axis_tx=time_axis_relative + time_axis_origin,
            ground_points=self.ground_point,
            frequencies_doppler_centroid=self.doppler_freq,
            wavelength=self.wavelength,
        )
