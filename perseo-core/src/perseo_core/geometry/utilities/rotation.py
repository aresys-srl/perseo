...
"""Rotation transformations utilities, thin wrapper on scipy.spatial.transform.Rotation to ease the manipulation
of Yaw Pitch Roll"""
...
def compute_rotation(
    ...
    """Compute the rotation defined by a rotation order and the yaw, pitch and roll angles.
...
def compute_euler_angles_from_rotation(order: RotationOrderLike, rotation: transform.Rotation) -> np.ndarray:
    """Compute euler angles (yaw, pitch and roll) from the rotation matrix and its rotation order.
