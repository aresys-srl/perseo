...
def precise_datetime_to_numpy(
    times: PreciseDateTime | NDArray[Shape["*"], PreciseDateTime],  # noqa: F722
) -> np.datetime64 | NDArray[Shape["*"], np.datetime64]:  # noqa: F722
    ...
    Parameters
    ----------
    date : PreciseDateTime | NDArray[Shape["*"], PreciseDateTime]
        times to be converted from PreciseDateTime format to numpy.datetime64[ns]

    Returns
    -------
    np.datetime64 | NDArray[Shape["*"], np.datetime64]
