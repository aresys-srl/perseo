...
from perseo_quality.core.generic_dataclasses import DecibelConversion, MaskingMethod, SideLobesDirections
...
def pslr_masking(
    ...
    # Compute pixel axes
    a_x = np.arange(0, data.shape[1]) - peak_pos[1]
    a_y = np.arange(0, data.shape[0]) - peak_pos[0]
...
def pslr_profile_cutting(
    ...
    # Compute pixel axes
    a_x = np.arange(0, masked_data.shape[1]) - peak_pos[1]
    a_y = np.arange(0, masked_data.shape[0]) - peak_pos[0]
...
def islr_masking(
    ...
    if mask_flag == MaskingMethod.PEAK:
        if lobes_flag:
            ...
            main_lobe_rng_size = (np.sum(rng_idx) - 1) / rng_res
            main_lobe_az_size = (np.sum(az_idx) - 1) / az_res
        else:
            main_lobe_mask = generate_peak_mask(data)
            ...
            # determine the width and length of the main lobe masking portion in terms of resolution cells
            main_lobe_rng_size = np.squeeze((rows_filled - 1) / rng_res)
            main_lobe_az_size = np.squeeze((cols_filled - 1) / az_res)
