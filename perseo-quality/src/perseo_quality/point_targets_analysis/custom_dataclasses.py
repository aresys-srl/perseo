...
class IRFDataOutput:
    ...
    ground_range_resolution: float = np.nan
    # pslr
    range_pslr: float = np.nan
    azimuth_pslr: float = np.nan
    pslr_2d: float = np.nan
...
class GenericInfoOutput:
    ...
    azimuth_position: float | None = None
...
class PTAdditionalInfo:
    ...
    orbit_direction: SAROrbitDirection | None = None  # sensor orbit direction
    peak_azimuth_time: PreciseDateTime | None = None  # azimuth time at signal peak coordinates
    peak_azimuth_from_burst_start: float | None = (
        None  # azimuth pixel position at signal peak coordinates relative to burst start
    )
    peak_range_time: float | None = None  # range time at signal peak coordinates
    look_angle: float | None = None  # look angle at nominal target coordinates
    ground_velocity: float | None = None  # ground velocity at nominal target coordinates
    doppler_rate_theoretical: float | None = None  # theoretical doppler rate at nominal target coordinates
    doppler_rate_real: float | None = None  # real (annotated) doppler rate at nominal target coordinates
    doppler_frequency: float | None = None  # doppler frequency from mechanical squint angle at signal peak coordinates
    steering_doppler_frequency: float | None = (
