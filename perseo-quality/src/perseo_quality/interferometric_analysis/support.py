...
def coherence_2d_histogram_computation_core(
    ...
    These discrete histograms are computed by splitting the input coherence array along a direction (azimuth or range)
    in several blocks and then computing the histogram of coherence for that block with a fixed number of coherence bins
    in a fixed range (so that all sub-blocks will match the same coherence intervals).
