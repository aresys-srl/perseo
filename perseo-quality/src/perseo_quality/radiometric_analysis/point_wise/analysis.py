...
from perseo_quality.core.custom_errors import CoordinatesOutOfBounds
...
def point_wise_radiometric_analysis(
    ...
    azimuth_times: list[PreciseDateTime | int] | None = None,
    range_times: list[float | int] | None = None,
    swath_name: str | None = None,
    selected_polarization: str | SARPolarization | None = None,
    is_pixel: bool = False,
    analysis_config: PointWiseRadiometricAnalysisConfig | None = None,
    ...
    azimuth_times : list[PreciseDateTime | int] | None, optional
    ...
    range_times : list[float | int] | None, optional
    ...
    analysis_config : PointWiseRadiometricAnalysisConfig | None, optional
    ...
    if analysis_config is None:
        analysis_config = PointWiseRadiometricAnalysisConfig()
    ...
    log.info(f"Requested output is {analysis_config.output_quantity.name.replace('_', ' ')}")
    ...
    for channel in product.channels_list:
        ...
        log.info(f"Input data is {channel_data.radiometric_quantity.name.replace('_', ' ')}")
    ...
    for channel in product.channels_list:
        ...
        az_mid_swath_pixel = channel_data.azimuth_axis.size // 2
        rng_mid_swath_pixel = channel_data.slant_range_axis.size // 2
        ...
        if analysis_config.direction in (
            RadiometricAnalysisDirection.ALL,
            RadiometricAnalysisDirection.RANGE,
        ):
            ...
            look_cond = _is_look_angles_computation_required(
                config=analysis_config, direction=RadiometricAnalysisDirection.RANGE
            )
            ...
            if analysis_config.axis == RadiometricAnalysisAxes.INCIDENCE_ANGLE:
                out_axis = np.rad2deg(incidence_angles)
            elif analysis_config.axis == RadiometricAnalysisAxes.LOOK_ANGLE:
                out_axis = np.rad2deg(look_angles)

            for az_idx, az_pixel_index in enumerate(azimuth_times_px):
                log.info(f"Processing data for azimuth pixel {az_pixel_index}...")
                ...
                target_area = channel_data.read_data(
                    ...
                    cropping_size=(channel_data.slant_range_axis.size, analysis_config.az_average_lines),
                )
                ...
                out_storage.append(
                    PointWiseRadiometricAnalysisOutput(
                        ...
                        axis_type=analysis_config.axis,
                        radiometric_quantity=analysis_config.output_quantity,
                        original_profile_db=original_profile,
                        smoothed_profile_db=smoothed_profile,
                        axis=out_axis,
                        time=str(az_time),
                    )
                )
        ...
        if analysis_config.direction in (
            RadiometricAnalysisDirection.ALL,
            RadiometricAnalysisDirection.AZIMUTH,
        ):
            ...
            for rng_idx, rng_pixel_index in enumerate(range_times_px):
                ...
                target_area = channel_data.read_data(
                    ...
                    cropping_size=(analysis_config.rng_average_samples, channel_data.azimuth_axis.size),
                )
                smoothed_profile, original_profile = _radiometric_swath_processing(
                    target_area=target_area,
                    input_quantity=channel_data.radiometric_quantity,
    ...
    if azimuth_times is not None:
        azimuth_times_px = [
            channel.times_to_pixel_conversion(
                azimuth_time=azimuth_times[idx], range_time=channel.mid_range_time, burst=bursts[idx]
            )[0]
...
def _get_times_from_pixels(
    ...
    -------
    np.ndarray | None
        azimuth times corresponding to input times or None
    np.ndarray | None
        range times corresponding to input times or None
    """
...
def _init_pixel_arrays(
    azimuth_times: np.ndarray | None, range_times: np.ndarray | None, is_pixel: bool
) -> tuple[np.ndarray | None, np.ndarray | None]:
    ...
    if azimuth_times is not None:
        ...
        if is_pixel:
            ...
            if len(main_type) > 1 or main_type.pop() is not PreciseDateTime:
                raise ra_err.PixelTimesMismatchError("Mismatch between pixel flag and input azimuth values")
...
def _is_incidence_angles_computation_required(
    ...
    conversion_cond = np.logical_and(
        input_quantity != config.output_quantity, config.value == RadiometricAnalysisValue.AMPLITUDE
    )
    # output axis in incidence angles
    out_angle_cond = np.logical_or(
        direction == RadiometricAnalysisDirection.RANGE, config.axis == RadiometricAnalysisAxes.INCIDENCE_ANGLE
    )
...
def _is_look_angles_computation_required(
    config: PointWiseRadiometricAnalysisConfig, direction: RadiometricAnalysisDirection
) -> bool:
    ...
    return np.logical_and(
        direction == RadiometricAnalysisDirection.RANGE, config.axis == RadiometricAnalysisAxes.LOOK_ANGLE
    )
...
def _radiometric_swath_processing(
    ...
    if config.value == RadiometricAnalysisValue.AMPLITUDE:
        if input_quantity != config.output_quantity:
            if direction == RadiometricAnalysisDirection.RANGE:
                incidence_angle_array = np.tile(incidence_angles, (config.az_average_lines, 1)).T
    ...
    elif config.value == RadiometricAnalysisValue.PHASE:
        if direction == RadiometricAnalysisDirection.RANGE:
