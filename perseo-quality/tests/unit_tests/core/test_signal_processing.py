...
raster_left = random_rng1.random((10, 10)) + random_rng1.random((10, 10)) * 1j
raster_right = random_rng1.random((10, 10)) + random_rng1.random((10, 10)) * 1j
...
class SignalProcessingTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.expected_ptar = -40.73099659417915
        self.expected_dtar = -39.925681943444204
        self.expected_enl = 1.8828847434343199

    def test_convert_to_db(self) -> None:
        """Testing convert_to_db function."""
        assert np.isnan(sp.convert_to_db(0))
        assert sp.convert_to_db(10) == 10
        assert sp.convert_to_db(10, mode=gdt.DecibelConversion.POWER) == 10
        assert sp.convert_to_db(10, mode=gdt.DecibelConversion.AMPLITUDE) == 20
        np.testing.assert_array_equal(sp.convert_to_db([10, 100, 1000]), [10, 20, 30])
        np.testing.assert_array_equal(
            sp.convert_to_db([10, 100, 1000], mode=gdt.DecibelConversion.AMPLITUDE),
            [20, 40, 60],
        )

    def test_locate_max_2d(self) -> None:
        """Testing locate_max_2d function."""
        mat = np.zeros((10, 10))
        max_id = (4, 8)
        mat[max_id] = 1
        assert sp.locate_max_2d(mat) == max_id

    def test_crop_array_2d(self) -> None:
    ...
    def test_modulate_data(self) -> None:
        ...
        assert np.dtype(np.complex128) is sp.modulate_data(mat, np.array(0)).dtype
        assert sp.modulate_data(mat, np.array(0)).sum() == 100
        assert np.abs(sp.modulate_data(mat, np.array(1000)).sum()) == 100
        assert mat.shape == mod.shape

    def test_modulate_data_float_frequency(self) -> None:
        """Testing modulate_data function."""
        mat = np.ones((10, 10))
        mod = sp.modulate_data(mat, 1000.0)
        assert np.dtype(np.complex128) is sp.modulate_data(mat, np.array(0)).dtype
        assert sp.modulate_data(mat, np.array(0)).sum() == 100
        assert np.abs(sp.modulate_data(mat, np.array(1000)).sum()) == 100
        assert mat.shape == mod.shape

    def test_modulate_data_array_frequency(self) -> None:
        """Testing modulate_data function."""
        mat = np.ones((10, 10))
        mod = sp.modulate_data(mat, np.full((10,), 1000.0))
        assert np.dtype(np.complex128) is sp.modulate_data(mat, np.array(0)).dtype
        assert sp.modulate_data(mat, np.array(0)).sum() == 100
        assert np.abs(sp.modulate_data(mat, np.array(1000)).sum()) == 100
        assert mat.shape == mod.shape

    def test_modulate_data_list_frequency(self) -> None:
        """Testing modulate_data function with list frequency."""
        mat = np.ones((10, 10))
        mod = sp.modulate_data(mat, [1000.0] * 10)
        assert np.dtype(np.complex128) is sp.modulate_data(mat, [0] * 10).dtype
        assert sp.modulate_data(mat, [0] * 10).sum() == 100
        assert np.abs(sp.modulate_data(mat, [1000.0] * 10).sum()) == 100
        assert mat.shape == mod.shape

    def test_compute_point_target_ambiguity_ratio_db(self) -> None:
        """Testing compute_point_target_ambiguity_ratio_db function."""
        ptar = sp.compute_point_target_ambiguity_ratio_db(
            point_target_roi=raster * 1e-6,
            right_ambiguity_roi=raster_right * 1e-8,
            left_ambiguity_roi=raster_left * 1e-8,
            interp_factor=8,
        )
        np.testing.assert_allclose(ptar, self.expected_ptar, atol=1e-5, rtol=0)

    def test_compute_distributed_target_ambiguity_ratio_db(self) -> None:
        """Testing compute_distributed_target_ambiguity_ratio_db function."""
        dtar = sp.compute_distributed_target_ambiguity_ratio_db(
            distributed_target_roi=raster * 1e-6,
            right_ambiguity_roi=raster_right * 1e-8,
            left_ambiguity_roi=raster_left * 1e-8,
        )
    ...
    def test_radiometric_correction_case0(self) -> None:
        ...
        np.testing.assert_allclose(
            converted_raster_abs,
            self.expected_radiometric_conversions[0],
            atol=self.tolerance,
            rtol=0,
        )

    def test_radiometric_correction_case1(self) -> None:
        ...
        np.testing.assert_allclose(
            converted_raster_abs,
            self.expected_radiometric_conversions[1],
            atol=self.tolerance,
            rtol=0,
        )

    def test_radiometric_correction_case2(self) -> None:
        ...
        np.testing.assert_allclose(
            converted_raster_abs,
            self.expected_radiometric_conversions[2],
            atol=self.tolerance,
            rtol=0,
        )

    def test_radiometric_correction_case3(self) -> None:
        ...
        np.testing.assert_allclose(
            converted_raster_abs,
            self.expected_radiometric_conversions[3],
            atol=self.tolerance,
            rtol=0,
        )

    def test_radiometric_correction_case4(self) -> None:
        ...
        np.testing.assert_allclose(
            converted_raster_abs,
            self.expected_radiometric_conversions[4],
            atol=self.tolerance,
            rtol=0,
        )

    def test_radiometric_correction_case5(self) -> None:
        ...
        np.testing.assert_allclose(
            converted_raster_abs,
            self.expected_radiometric_conversions[5],
            atol=self.tolerance,
            rtol=0,
        )
...
def measure_frequency_shift(signal1, signal2, df) -> float:
    ...
    correlation = scipy.signal.correlate(spectrum1, spectrum2, mode="full")
    lags = scipy.signal.correlation_lags(spectrum1.size, spectrum2.size, mode="full")
...
class ShiftSpectrumTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.df = 1 / (self.t[-1] - self.t[0])  # Frequency step

        # Create signals
        self.signal1 = 1 + np.sinc(self.t)  # Original sinc function
        self.signal2 = 1 + 0.5 * np.sinc(self.t / 2)  # Sinc function with half the bandwidth
        self.signal3 = 1 + 0.25 * np.sinc(self.t / 4)  # Sinc function with quarter the bandwidth

        # Stack the three signals into a matrix of shape (50, 3)
        self.signal = np.stack((self.signal1, self.signal2, self.signal3), axis=1)

    def test_shift_spectrum_single_signal(self) -> None:
        """Test shift_spectrum with a single signal."""
        single_signal = self.signal[:, 0]

        frequency_shift = 0.5
        shifted_signal = sp.shift_spectrum(data=single_signal, shift=frequency_shift * self.dt)
    ...
    def test_shift_spectrum_matrix_axis_0(self) -> None:
        ...
        frequency_shift = 5
        shifted_signal = sp.shift_spectrum(data=self.signal, shift=frequency_shift * self.dt, axis=0)
    ...
    def test_shift_spectrum_matrix_axis_1(self) -> None:
        """Test shift_spectrum with a matrix along axis 1."""
        frequency_shift = 5
        shifted_signal = sp.shift_spectrum(data=self.signal.T, shift=frequency_shift * self.dt, axis=1).T
    ...
    def test_shift_spectrum_with_array_shift(self) -> None:
        """Test shift_spectrum with an array of shifts."""
        frequency_shift = np.array([0.25, 0.4, 0.6])
        shifts = frequency_shift * self.dt
        shifted_signal = sp.shift_spectrum(data=self.signal, shift=shifts, axis=0)
    ...
    def test_shift_spectrum_with_list_shift(self) -> None:
        """Test shift_spectrum with a list of shifts."""
        frequency_shift = np.array([0.25, 0.4, 0.6])
        shifts = (frequency_shift * self.dt).tolist()
        shifted_signal = sp.shift_spectrum(data=self.signal.T, shift=shifts, axis=1).T
